...
int board_init(void)
{
	...
	gpio_request(WIFI_LVL_GPIO, "wifi_lvl_gpio");
	gpio_direction_output(WIFI_LVL_GPIO, 1);

	/* Enable 5V circuits */
	gpio_request(POWER_CONNECTORS, "power_connectors");
	gpio_direction_output(POWER_CONNECTORS, 1);
	gpio_request(POWER_E0_EN, "power_e0_en");
	gpio_direction_output(POWER_E0_EN, 1);
	gpio_request(POWER_E1_EN, "power_e1_en");
	gpio_direction_output(POWER_E1_EN, 1);
	gpio_request(POWER_C_EN, "power_c_en");
	gpio_direction_output(POWER_C_EN, 1);

	/* Start MSP in application mode */
	gpio_request(MSP_BSL_GPIO, "msp_bsl");
	gpio_direction_output(MSP_BSL_GPIO, 0);
	gpio_request(MSP_TEST_GPIO, "test_test");
	gpio_direction_output(MSP_TEST_GPIO, 0);
	gpio_request(MSP_RESET_GPIO, "msp_reset");
	gpio_direction_output(MSP_RESET_GPIO, 0);
	udelay(50000);
	gpio_set_value(MSP_RESET_GPIO, 1);
